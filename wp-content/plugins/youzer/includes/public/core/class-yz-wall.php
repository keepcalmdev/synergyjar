...
class Youzer_Wall {
	...
	function get_wall_post_file( $attachments, $activity_id ) { 
		...
		<div class="yzw-file-post">
			<?php 
			$file_excerpt = yz_get_filename_excerpt( $data[0], 45 );
			$file_type = $file_excerpt['type'];
			if($file_type === 'application/pdf'){ ?>

				<object data="<?php echo yz_get_media_url( $attachments[0] ); ?>" type="application/pdf" width="100%" height="600"> <a href="<?php echo yz_get_media_url( $attachments[0] ); ?>">test.pdf</a></object>

			<?php } else {	?>
				<div class="yzw-file-post-other">
    ...
    function scripts() {
		...
		//wp_localize_script( 'yz-wall', 'Youzer', $youzer_vars );
