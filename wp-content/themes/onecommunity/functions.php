...
if (function_exists('register_sidebar')) {
	...
	function onecommunity_widgets_init()
	{
		...
		register_sidebar(array(
			'name' => 'Sidebar - BP Profiles',
			'id'   => 'sidebar-profiles',
			'description'   => 'This is a widgetized area visible on the buddypress profiles.',
			'before_widget' => '<div class="sidebar-box %2$s widget" id="%1$s">',
			'after_widget'  => '</div><!--sidebar-box ends--><div class="clear"></div>',
			'before_title'  => '<div class="sidebar-title">',
			'after_title'   => '</div>'
		...
		register_sidebar(array(
			'name' => 'Sidebar - BP Profiles (Cached)',
...
function onecommunity_scripts()
{
	...
	// Theme stylesheet.
	wp_enqueue_style('onecommunity-style', get_stylesheet_uri());
...
function onecommunity_fonts_url()
{
	...
	wp_enqueue_style(get_theme_mod('onecommunity_font_name', 'Nunito'), get_theme_mod('onecommunity_font_url', '//fonts.googleapis.com/css?family=Nunito:400,400i,600,700,800,900&display=swap'));
...
function register_menus()
{
	...
	register_nav_menus(
		array(
			'mini-footer-menu-more' => 'Mini footer menu More'
		)
	);
...
?>
	...
	{
		global $post;
		return ' ... <a class="more-link" href="' . get_permalink($post->ID) . '">' . esc_attr__('Continue reading &rarr;', 'onecommunity') . '</a>';
	}
	...
	if (!function_exists('ajax_login')) {
		function ajax_login()
		{
			...
			if (is_wp_error($user_signon)) {
				...
				echo json_encode(array('loggedin' => true, 'message' => esc_attr__('Login successful, redirecting...', 'onecommunity'), 'username' => $user_signon->data->user_login));
			}
			wp_die();
		}
	}
	...
	function onecommunity_blog_classic()
	{
		...
		if ($blog_posts_type == 1) {
			...
			$wp_query->query('posts_per_page=3' . '&paged=' . $page);
			...
			$wp_query = null;
			$wp_query = new WP_Query();
			$wp_query->query('orderby=meta_value_num&meta_key=_liked&posts_per_page=3' . '&paged=' . $page);
			while ($wp_query->have_posts()) : $wp_query->the_post();
			...
			$wp_query = null;
			$wp_query = new WP_Query();
			$wp_query->query('orderby=comment_count&posts_per_page=3' . '&paged=' . $page);
			while ($wp_query->have_posts()) : $wp_query->the_post();
	...
	function onecommunity_blog_archive_tabs()
	{
		...
		if ($blog_posts_type == 1) {
			...
			$wp_query = null;
			$wp_query = new WP_Query();
			$wp_query->query('posts_per_page=6&' . $taxonomy . '=' . $term_id . '&paged=' . $page);
			while ($wp_query->have_posts()) : $wp_query->the_post();
			...
			$wp_query = null;
			$wp_query = new WP_Query();
			$wp_query->query('orderby=meta_value_num&meta_key=_liked&posts_per_page=6&' . $taxonomy . '=' . $term_id . '&paged=' . $page);
			while ($wp_query->have_posts()) : $wp_query->the_post();
			...
			$wp_query = null;
			$wp_query = new WP_Query();
			$wp_query->query('orderby=comment_count&posts_per_page=6&' . $taxonomy . '=' . $term_id . '&paged=' . $page);
			while ($wp_query->have_posts()) : $wp_query->the_post();
	...
	if (get_theme_mod('onecommunity_live_notifications_enable', false) == true) {
		...
		function onecommunity_notifications_list()
		{
			...
			if ($notifications) {
				$counter = 0;
				for ($i = 0, $count = count($notifications); $i < $count; ++$i) { ?><li class="notification"><?php echo wp_kses($notifications[$i], array('a' => array('href' => array(), 'title' => array()))); ?></li><?php $counter++;
																																																													}
																																																												} ?><li class="view-all"><a href="<?php echo bp_loggedin_user_domain() ?>notifications"><?php esc_attr_e("Manage", "onecommunity"); ?></a></li>
		...
		function onecommunity_messages_list()
		{
			...
			if (bp_has_message_threads('max=3&user_id=' . get_current_user_id() . '')) : ?><ul><?php while (bp_message_threads()) : bp_message_thread(); ?><li class="<?php if (bp_message_thread_has_unread()) { ?>unread<?php } ?>">
							<div class="avatar"><?php bp_message_thread_avatar() ?></div>
							<div class="message-content">
								<div class="message-content-details"><?php bp_message_thread_from() ?> <?php bp_message_thread_last_post_date() ?></div><a href="<?php bp_message_thread_view_link() ?>" class="message-title"><?php bp_message_thread_subject() ?></a>
								<p><?php bp_message_thread_excerpt() ?></p>
							</div>
							<div class="clear"></div>
						</li><?php endwhile; ?><li class="view-all"><a href="<?php echo bp_loggedin_user_domain() ?>messages"><?php esc_attr_e('View all messages', 'onecommunity'); ?></a></li>
				</ul><?php else : ?><div id="message" class="info">
