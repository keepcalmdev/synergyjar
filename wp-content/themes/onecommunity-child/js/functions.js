(function ($) {
	...
	if ($('#profile-edit-form').length) {
		...
		$('.field_address input, .field_city input').on('change', function () {

			let addressInput = $('.field_address input');
			let cityInput = $('.field_city input');
			let address = addressInput.val();
			let city = cityInput.val();

			//let state = $('.field_state select option:selected').val();
			const errorMessage = '<div class="bp-messages bp-feedback error geocode">' +
				'<span class="bp-icon" aria-hidden="true"></span>' +
				'<p>Sorry, that address doesn\'t exist!</p>' +
				'</div>';

			if (address.length && city.length) {

				//let query = http_build_query({address: address + ',' + city + ',' + state, key: GoogleGeocode.geocode_key});
				let query = http_build_query({ address: address + ',' + city, key: GoogleGeocode.geocode_key });
				...
				$.ajax({
					...
					success: function (data) {
						if (data.status === 'OK') {
							...
							$('.field_coordinates input').val('lat: ' + lat + ', lng: ' + lng);

						} else if (data.status === 'ZERO_RESULTS') {

							$(errorMessage).insertBefore(addressInput);
							$(errorMessage).insertBefore(cityInput);
							addressInput.val('');
							cityInput.val('');

							setTimeout(function () {
								$('.bp-messages.bp-feedback.error.geocode').remove();
							}, 6000);
...
if (document.getElementById('header-search-mobile')) {
	document.getElementById('header-search-mobile-icon').addEventListener('click', function () {
		document.querySelector('#header-search-mobile-icon').style.display = "none";
		let prnt = document.querySelector('#header-search-mobile');
		prnt.classList.toggle('expand-search');
